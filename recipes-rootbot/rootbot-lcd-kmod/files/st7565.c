...
long st7565_control(struct file *f, unsigned int control, unsigned long value);
...
#ifdef DEBUG
	printk("[%s] received %i chars: %s\n", __FUNCTION__, len, rx_buffer);
#endif
...
long st7565_control(struct file *f, unsigned int control, unsigned long value)
{
	if (control == IOCTL_LCD_CLEAR_ALL)
	{
#ifdef DEBUG
