...
get_rcn_kernel_header_package () {
	...
	wget ${OPTS} --directory-prefix="${DDIR}" "${RURL}/"
	...
	if [ -n "${HDR_DEB}" ]; then
		...
		wget ${OPTS} --directory-prefix="${DDIR}" "${RURL}/${HDR_DEB}"
	else
	...
	if [ -n "${KPATCH}" ]; then
		...
		wget ${OPTS} --directory-prefix="${DDIR}" "${RURL}/${KPATCH}"
	else
...
get_official_kernel_source () {	
	if [ -n "$(echo ${MAIN_KVER} | sed -nE 's/(3\.)+.*/\1/p')" ]; then
		...
		MAIN_KERNEL_URL="${OFFICIAL_KERNEL_BASE_URL}/v$(echo ${MAIN_KVER} | sed -nE 's/([0-9]+\.[0-9]+).*/\1/p')/linux-${MAIN_KVER}.tar.gz"
	fi

	notif "getting official kernel source from ${MAIN_KERNEL_URL}..."
	wget ${OPTS} --directory-prefix="${DDIR}" "${MAIN_KERNEL_URL}"

	notif "unpacking official kernel source..."
	sleep 1
 
 echo tar xvfz "${DDIR}/linux-${MAIN_KVER}.tar.gz" --directory "${DDIR}"
	tar xvfj "${DDIR}/linux-${MAIN_KVER}.tar.gz" --directory "${DDIR}"
}
